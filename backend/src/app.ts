...
app.use(cors({
  origin: 'http://172.16.153.98:4200', // или true для разработки
  credentials: true,
...
app.use((req, res, next) => {
  ...
  if (origin && allowedOrigins.includes(origin)) {
    ...
    if (origin && (origin.includes('172.16.153.98:4200') || origin.includes('127.0.0.1:4200'))) {
      res.header('Access-Control-Allow-Origin', origin);
      res.header('Access-Control-Allow-Credentials', 'true');
    }
  }
